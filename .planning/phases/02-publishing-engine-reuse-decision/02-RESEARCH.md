...
1. The spike should start with self-hosted Postiz and official docs, not Postiz Cloud as a primary dependency. Cloud can be referenced only if needed to clarify docs gaps, because LocalPilot's reuse decision must account for self-hosting burden, token residency, and operator fit [CITED: https://docs.postiz.com/introduction] [CITED: https://docs.postiz.com/quickstart] [CITED: https://docs.postiz.com/installation/development].
2. The phase should evaluate both TikTok `DIRECT_POST` and `UPLOAD`/draft-style fallback flows, but the planner must not assume either one is universally available. The reuse decision should treat upload/draft fallback as the safer baseline and direct post as an optional gate [CITED: https://docs.postiz.com/public-api/providers/tiktok] [CITED: https://developers.tiktok.com/doc/content-posting-api-reference-direct-post?enter_method=left_navigation].
3. LocalPilot must not assume a one-to-one mapping from its approval snapshot to Postiz state; the spike should explicitly validate whether LocalPilot's approved payload version can remain immutable while Postiz handles the publish-side state transitions. If the mapping cannot preserve the approved version boundary, that is a no-go signal for using Postiz as anything more than a thin adapter [CITED: https://docs.postiz.com/public-api/posts/create] [CITED: https://github.com/gitroomhq/postiz-app/libraries/nestjs-libraries/src/database/prisma/posts/posts.repository.ts].
4. Postiz's CLI and MCP surfaces are developer-only spike and automation tooling for this phase, not the production runtime publishing path. LocalPilot's runtime path must remain API-backed and keep production merchant publishing inside its backend-owned workflow boundary [CITED: https://docs.postiz.com/cli/introduction] [CITED: https://docs.postiz.com/cli/authentication] [CITED: https://docs.postiz.com/introduction].
