...
def get_valid_credential(conn, page_id, merchant_id, buffer_seconds=60):
    credential_row = store.get_facebook_page_token_row(conn, page_id, merchant_id)
    if credential_row is None:
        raise store.StoreError(404, f"Facebook credential not found for page {page_id}.")
    ...
    if expires_at is not None:
        ...
        if seconds_remaining <= buffer_seconds:
            raise store.StoreError(401, "Facebook credential expired.")
        if seconds_remaining <= 300:
            logger.warning(
                "facebook_credential_expiring_soon merchant_id=%s page_id=%s expires_in_seconds=%s",
                merchant_id,
                page_id,
                max(0, int(seconds_remaining)),
            )

    return token_crypto.decrypt_secret(credential_row["ciphertext"])
